---
title: "DAR F22 Project Fairness Auditor Assignment 2 Notebook"
...
* RCS ID: powerh
* Project Name: Fairness Auditor
* Summary of work since last week 

    * Trained and tested a random forest model
    * Found Equalized Odds and Balanced Accuracy for random forest model
    * Found Theil Index and Disparate Impact scores for baseline and reweighing models

* NEW: Summary of github issues added and worked

    * Having trouble calling aif360 from R, seems to be loading the python library to use in R but having trouble getting it to load
      * Karan aware of issue and working on it
    
* Summary of github commits 

    * powerh-09142022
    * Committed assignment 2
    
* List of presentations, papers, or other outputs

    * A General Framework for Fair Regression: https://arxiv.org/pdf/1810.05041.pdf
        * Fairness techniques for kernel regression with a focus on decision trees
    * A Convex Framework for Fair Regression: https://arxiv.org/pdf/1706.02409.pdf
        * Fairness regularizers for linear and logistic regression
    * Controlling Attribute Effect in Linear Regression: https://ieeexplore.ieee.org/document/6729491
        * Uses propensity modeling and constraints to control bias in linear regression
    * Fairness-Aware Learning through Regularization Approach: https://ieeexplore.ieee.org/document/6137441
        * Regularization for fairness applied to logistic regression
...
Evaluating Balanced Accuracy and Equalized Odds on a random forest model.
```{r}
# 1. Train ML model here on train data
train.scale$income <- as.factor(train.scale$income)
rfModel <- randomForest(income~., data=train.scale)
```

```{r}
# 2. Get predictions on test data
rfModel.pred <- predict(rfModel, test.scale)
# rfModel.pred <- ifelse(rfModel.prob > 0.5, 1, 0)
```

```{r}
# 3. Evaluate balanced accuracy
rfModel.bal_acc <- bacc(as.factor(test.scale$income), as.factor(rfModel.pred))
```

```{r}
# 4. Evaluate equalized odds
test2.rf <- test.scale
test2.rf$pred <- rfModel.pred
test2.rf$income <- as.factor(test2.rf$income)
test2.rf$gender <- as.factor(test2.rf$gender)

# Evaluate TPR difference
# NOTE: In the library `fairness`, Equalized Odds is defined as separation which is 
# the TPR difference only. This is not the same Equalized Odds calculated here.
tpr_results <- equal_odds(data         = test2.rf, 
                     outcome      = 'income', 
                     outcome_base = '0', 
                     group        = 'gender',
                     preds        = 'pred',
...
# Evaluate FPR difference
fpr_results <- fpr_parity(data         = test2.rf, 
                     outcome      = 'income', 
                     outcome_base = '0', 
                     group        = 'gender',
                     preds        = 'pred',
...
# Evaluate Equalized Odds (EO)
rfModel.eo <- max(abs(tpr_diff), abs(fpr_diff))
```

We compare the Balanced Accuracy and Equalized Odds scores for the random forest model, baseline model, and reweighing model.

```{r}
# 5. Compare results with baselineModel and reweighingModel
print("Balanced Accuracy scores")
print(paste0("Random forest model: ", rfModel.bal_acc))
print(paste0("Baseline model: ", baselineModel.bal_acc))
print(paste0("Reweighing model: ", reweighingModel.bal_acc))

print("Equalized Odds score")
print(paste0("Random forest model: ", rfModel.eo))
print(paste0("Baseline model: ", baselineModel.eo))
print(paste0("Reweighing model: ", reweighingModel.eo))
```

*Finding:* Of the three models, the baseline model has the best Balanced Accuracy, followed closely by the random forest model and then the reweighing model. As for Equalized Odds, the baseline and random forest models perform nearly the same. The reweighing model outperforms both and is closest to being considered fair.
...
Theil Index and Disparate Impact
```{r}
...
# Evaluate TPR difference
pp_results <- prop_parity(data         = test2, 
                     outcome      = 'income', 
                     outcome_base = '0', 
                     group        = 'gender',
                     probs        = 'prob', 
                     cutoff       = 0.5, 
                     base         = '0')  
pp_diff <- pp_results$Metric[1] - pp_results$Metric[4]

# Evaluate FPR difference
ap_results <- acc_parity(data         = test2,
                     outcome      = 'income',
                     outcome_base = '0',
                     group        = 'gender',
                     probs        = 'prob',
                     cutoff       = 0.5,
                     base         = '0')
ap_diff <- ap_results$Metric[1] - ap_results$Metric[4]

# Evaluate TPR difference
pp_results2 <- prop_parity(data         = test3, 
                     outcome      = 'income', 
                     outcome_base = '0', 
                     group        = 'gender',
                     probs        = 'prob', 
                     cutoff       = 0.5, 
                     base         = '0')  
pp_diff2 <- pp_results2$Metric[1] - pp_results2$Metric[4]

# Evaluate FPR difference
ap_results2 <- acc_parity(data         = test3,
                     outcome      = 'income',
                     outcome_base = '0',
                     group        = 'gender',
                     probs        = 'prob',
                     cutoff       = 0.5,
                     base         = '0')
ap_diff2 <- ap_results2$Metric[1] - ap_results2$Metric[4]
```


```{r}
# 3. Compare the scores and explain what you find
print("Proportional Parity")
print(paste0("baseline model: ", pp_diff))
print(paste0("reweighing model: ", pp_diff2))

print("Accuracy Parity")
print(paste0("baseline model: ", ap_diff))
print(paste0("reweighing model: ", ap_diff2))
```
```{r}
print("Probability Density Plot")
pp_results$Probability_plot
pp_results2$Probability_plot
```

*Finding:* For proportional parity, the measure is the proportion of positive predictions within a subgroup so we hope to see values close to 0 where all subgroups see an equal proportion of positive predictions. The reweighing model outperforms the baseline model for this score and could be considered fair.

As for accuracy parity, the measure is the proportion of correct predictions within a subroup so we hope to see values close to 0 where the model has similar accuracy between subgroups. Neither model is fair and perform about equivalent to each other.

For the probability density plots, we hope to see similar shapes between the subgroups to indicate a similar amount of people predicted to have a positive outcome. The baseline model's plot has a very different shape between subgroups but the reweighing model's plot indicates closer distributions between subgroups. This is reflected in the proportional parity measure.
