...
library(MASS)
#we choose to train LDA model here
ldaModel = lda(income ~ ., data=train.scale, prior=c(1,1)/2)
# 2. Get predictions on test data
ldaModel1 <- predict(ldaModel, test.scale, type = 'response')
ldaModel.prob <- ldaModel1$posterior[,2]
ldaModel.pred <- ifelse(ldaModel.prob > 0.5, 1, 0)

# 3. Evaluate balanced accuracy
# Calculate Balanced Accuracy
ldaModel.bal_acc <- bacc(as.factor(test.scale$income), as.factor(ldaModel.pred))
```
...
# Create a copy of the dataset for fairness evaluation
test4 <- test.scale
test4$prob <- ldaModel.prob    #as.numeric(levels(ldaModel.pred))[ldaModel.pred]
...
# Evaluate Equalized Odds (EO)
ldaModel.eo <- max(abs(tpr_diff), abs(fpr_diff))
...
print(paste0("Baseline model: ", baselineModel.bal_acc))
print(paste0("Reweighing model: ", reweighingModel.bal_acc))
print(paste0("LDA model: ", ldaModel.bal_acc))

print("# Equalized Odds scores")
print(paste0("Baseline model: ", baselineModel.eo))
print(paste0("Reweighing model: ", reweighingModel.eo))
print(paste0("LDA model: ", ldaModel.eo))
```
...
# I choose to use Proportional parity and Predictive rate parity

# 2. Measure them on baselineModel and reweighingModel
baseline_prop <- prop_parity(data         = test2, 
                     outcome      = 'income', 
                     outcome_base = '0', 
                     group        = 'gender',
                     probs        = 'prob', 
                     cutoff       = 0.5, 
                     base         = '0')
reweighing_prop <- prop_parity(data         = test3, 
                     outcome      = 'income', 
                     outcome_base = '0', 
                     group        = 'gender',
                     probs        = 'prob', 
                     cutoff       = 0.5, 
                     base         = '0')
knitr::kable(baseline_prop$Metric)
knitr::kable(reweighing_prop$Metric)


baseline_pred_parity <- pred_rate_parity(data         =test2, 
                     outcome      = 'income', 
                     outcome_base = '0', 
                     group        = 'gender',
                     probs        = 'prob', 
                     cutoff       = 0.5, 
                     base         = '0')
reweighing_pred_parity <- pred_rate_parity(data         =test3, 
                     outcome      = 'income', 
                     outcome_base = '0', 
                     group        = 'gender',
                     probs        = 'prob', 
                     cutoff       = 0.5, 
                     base         = '0')
knitr::kable(baseline_pred_parity$Metric)
knitr::kable(reweighing_pred_parity$Metric)
# 3. Compare the scores and explain what you find

```

*Finding:*  First, we see the reult of Proportional parity in baseline and reweighing models. If the difference of the proportion between two subgroup is smaller, it is better. Thus, we can see in this case, reweighing models did better in Proportional parity. Then, we look at Predictive rate parity metric. The difference of precision is smaller in baseline model than that in reweighing models, which is better.

### 3) List fairness libraries
aif360(it contains many metrics for datasets and models to test for biases)

fairmodels(checking the bias toward different attributes like gender, race, nationality, etc.)

FairMclus(providing methods preventing bias in classification due to gender, race and so on)
